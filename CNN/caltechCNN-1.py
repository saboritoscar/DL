...
history = nn.fit(X_train, y_train, batch_size=Batch_size, epochs=Num_epochs, verbose=1,validation_split=0.1)
...
plt.savefig('./model2/3L-adam_cnn_accuracy'+'_'+str(Num_epochs)+'_'+str(Batch_size)+'_'+str(dense_size)+'.pdf')
plt.close()
#Loss plot
...
plt.xlabel('epoch')
plt.legend(['train','val'], loc='upper left')
plt.savefig('./model2/3L-adam_cnn_loss'+'_'+str(Num_epochs)+'_'+str(Batch_size)+'_'+str(dense_size)+'.pdf')
...
pl.colorbar()
pl.savefig('./model2/adam_cnn_confmat'+'_'+str(Num_epochs)+'_'+str(Batch_size)+'_'+str(dense_size)+'.pdf')
...
nn_json = nn.to_json()
with open('./models/adam_cnn_confmat'+'_'+str(Num_epochs)+'_'+str(Batch_size)+'_'+str(dense_size)+'.json', 'w') as json_file:
        json_file.write(nn_json)
weights_file = "weights-MNIST_"+str(score[1])+".hdf5"
nn.save_weights(weights_file,overwrite=True)

#Loading model and weights
json_file = open('./models/adam_cnn_confmat'+'_'+str(Num_epochs)+'_'+str(Batch_size)+'_'+str(dense_size)+'.json','r')
